...
> GitHub에 새로 올라오는 Agent 프레임워크 실시간 인덱스 — 스타 수가 아닌 최신순 정렬
...
GitHub에서 새로운 Agent 프레임워크 저장소를 지속적으로 발견하고 인덱싱하는 가벼운 트래커입니다. 스타 수가 많은 기존 프로젝트 위주로 편향된 큐레이션 리스트와 달리, 이 도구는 프레임워크가 공개되는 당일에 바로 포착해서 개발자들이 새로운 선택지를 빠르게 파악할 수 있게 해줍니다.

이 목록은 GitHub Actions cron에 의해 **15분마다 자동 업데이트**됩니다. 각 커밋은 업스트림 데이터 소스의 실제 변경 사항을 반영합니다——새 항목 추가, 만료 항목 제거——그래서 보이는 내용이 항상 최신임을 신뢰할 수 있습니다.
...
2. `data/items.json`(이전 스냅샷)과 diff를 냅니다.
